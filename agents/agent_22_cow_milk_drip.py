...
def cow_milk_drip(obs):
    try:
        ...
        s = _state.setdefault(player, {"cows_day0": False, "cow_day4": False, "pastures_built": False})

        if not s["cows_day0"]:
            market_actions.append(["BUY_ANIMAL", "COW", 2])
            market_actions.append(["HIRE"])
            s["cows_day0"] = True
        if day >= 4 and not s["cow_day4"]:
        ...
        if seeds.get("WHEAT", 0) < len(empty_tiles) + 2:
            need = len(empty_tiles) + 2 - seeds.get("WHEAT", 0)
            market_actions.append(["BUY_SEED", "WHEAT", need])

        # Sell milk (drip)
        milk_market_inv = market_inv.get("MILK", 0)
        milk_rate = 1 if milk_market_inv > 20 else 3
        ...
        # Farmer: FEED > CARE > HARVEST milk > COLLECT_FERTILIZER > plant/water/harvest wheat
        cur_tile = farm["tiles"][fy][fx] if 0 <= fy < len(farm["tiles"]) and 0 <= fx < len(farm["tiles"][fy]) else None
