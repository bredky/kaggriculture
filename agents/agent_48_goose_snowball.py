...
def goose_snowball(obs):
    try:
        ...
        s = _state.setdefault(player, {"geese_count": 0, "init": False})
        ...
        # Progressive goose buy thresholds: 300, 400, 500, 600, 700, 800, 900, 1000, 1100 (up to 10)
        thresholds = [300, 400, 500, 600, 700, 800, 900, 1000, 1100]
        gc = s["geese_count"]
        if gc < 10 and gc <= len(thresholds):
            threshold = thresholds[gc - 1]
            if money > threshold:
                market_actions.append(["BUY_ANIMAL", "GOOSE", 1])
        ...
        wheat_target = min(s["geese_count"] * 2, 20)
        empty_tiles = [(x, y) for x, y, t in all_tiles if t is None]

        if seeds.get("WHEAT", 0) + wheat_count < wheat_target:
            need = wheat_target - wheat_count - seeds.get("WHEAT", 0)
            if need > 0:
        ...
        # Farmer: FEED > CARE > HARVEST > COLLECT_FERTILIZER > wheat
        cur_tile = farm["tiles"][fy][fx] if 0 <= fy < len(farm["tiles"]) and 0 <= fx < len(farm["tiles"][fy]) else None
