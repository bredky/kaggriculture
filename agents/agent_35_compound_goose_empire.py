...
def compound_goose_empire(obs):
    try:
        ...
        # Buy more geese every 3 days if affordable (up to 20)
        if day - s["last_buy_day"] >= 3 and s["geese_count"] < 20 and money >= 400:
            market_actions.append(["BUY_ANIMAL", "GOOSE", 2])
            s["geese_count"] = min(20, s["geese_count"] + 2)
            s["last_buy_day"] = day
        ...
        # Sell all eggs and fertilizer daily
        if shed.get("EGG", 0) > 0:
