...
def sheep_wool_wheat(obs):
    try:
        ...
        s = _state.setdefault(player, {"sheep_day0": False, "sheep_day3": False, "hands_hired": False})

        if not s["sheep_day0"]:
            market_actions.append(["BUY_ANIMAL", "SHEEP", 2])
            market_actions.append(["HIRE"])
            s["sheep_day0"] = True
            s["hands_hired"] = True
        if day >= 3 and not s["sheep_day3"]:
            market_actions.append(["BUY_ANIMAL", "SHEEP", 2])
            s["sheep_day3"] = True
        ...
        target_wheat = 8
        if seeds.get("WHEAT", 0) + wheat_count < target_wheat:
            need = target_wheat - wheat_count - seeds.get("WHEAT", 0)
            if need > 0:
                market_actions.append(["BUY_SEED", "WHEAT", need])

        # Sell wool (max 2/day, hold if < $100)
        wool_price = prices.get("WOOL", 0)
        if shed.get("WOOL", 0) > 0 and wool_price >= 100:
            market_actions.append(["SELL", "WOOL", min(2, shed["WOOL"])])
        ...
        # Sell fertilizer
        if shed.get("FERTILIZER", 0) > 0 and prices.get("FERTILIZER", 0) >= 80:
            market_actions.append(["SELL", "FERTILIZER", min(2, shed["FERTILIZER"])])

        # Farmer: FEED > CARE > HARVEST > COLLECT_FERTILIZER > wheat farming
        cur_tile = farm["tiles"][fy][fx] if 0 <= fy < len(farm["tiles"]) and 0 <= fx < len(farm["tiles"][fy]) else None
        ...
        _asetup = _animal_setup(fx, fy, all_tiles, farm, inv, shed, "SHEEP", "PASTURE")
