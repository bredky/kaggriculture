...
def fertilizer_seller(obs):
    ...
    s = _state.setdefault(player, {"geese_bought": 0, "fert_sold_today": 0, "last_day": -1})
    if day != s["last_day"]:
        s["fert_sold_today"] = 0
        s["last_day"] = day

    if day == 0 and s["geese_bought"] == 0:
        market_actions.append(["BUY_ANIMAL", "GOOSE", 3])
        s["geese_bought"] = 3
    elif day == 1 and s["geese_bought"] < 5:
    ...
    if fert_in_shed > 0 and s["fert_sold_today"] < 4:
        sell_qty = min(fert_in_shed, 4 - s["fert_sold_today"])
        market_actions.append(["SELL", "FERTILIZER", sell_qty])
        s["fert_sold_today"] += sell_qty
    ...
    coops = [(x, y, t) for x, y, t in all_tiles if isinstance(t, dict) and t.get("kind") == "COOP"]
