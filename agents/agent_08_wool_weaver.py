...
def wool_weaver(obs):
    ...
    s = _state.setdefault(player, {"sheep_bought": 0, "wool_sold_today": 0, "last_day": -1})
    if day != s["last_day"]:
        s["wool_sold_today"] = 0
        s["last_day"] = day

    if day == 0 and s["sheep_bought"] == 0:
        market_actions.append(["BUY_ANIMAL", "SHEEP", 2])
        s["sheep_bought"] = 2
    elif day == 2 and s["sheep_bought"] < 3:
    ...
    wool_price = prices.get("WOOL", 100)
    wool_in_shed = shed.get("WOOL", 0)
    if wool_in_shed > 0 and wool_price >= 100 and s["wool_sold_today"] < 2:
        sell_qty = min(wool_in_shed, 2 - s["wool_sold_today"])
        market_actions.append(["SELL", "WOOL", sell_qty])
        s["wool_sold_today"] += sell_qty
