...
def melon_price_sniper(obs):
    try:
        ...
        s = _state.setdefault(player, {"big_sell_done": False, "opp_harvest_day": None})
        ...
        if seeds.get("WHEAT", 0) < 5:
            market_actions.append(["BUY_SEED", "WHEAT", 5 - seeds.get("WHEAT", 0)])

        # Detect opponent melon harvest day
        farms = obs["farms"]
        opp = 1 - player
        if opp < len(farms) and s["opp_harvest_day"] is None:
            opp_tiles = []
            for y, row in enumerate(farms[opp]["tiles"]):
                for x, tile in enumerate(row):
                    opp_tiles.append((x, y, tile))
            opp_melons = [(x, y, t) for x, y, t in opp_tiles if isinstance(t, dict) and t.get("crop") == "MELON"]
            if opp_melons:
                avg_planted = sum(t.get("planted_day", 0) for _, _, t in opp_melons) / len(opp_melons)
                s["opp_harvest_day"] = int(avg_planted) + 10

        opp_harvest = s["opp_harvest_day"]
        ...
        harvest_day = 10  # first melon plants mature at day 10

        if day >= 27:
            if melon_shed > 0:
                market_actions.append(["SELL", "MELON", melon_shed])
        elif not s["big_sell_done"] and melon_shed > 0 and day >= harvest_day:
            # Sell all at once on first harvest, unless opp harvest overlaps
            sell_day = harvest_day
            if opp_harvest is not None and abs(opp_harvest - harvest_day) <= 1:
                sell_day = harvest_day - 1
            if day >= sell_day:
                market_actions.append(["SELL", "MELON", melon_shed])
                s["big_sell_done"] = True
        elif s["big_sell_done"] and melon_shed > 0:
            market_actions.append(["SELL", "MELON", min(3, melon_shed)])

        # Farmer: water > harvest (at max_yield) > plant
