...
def goose_fertilizer_melon(obs):
    try:
        player = obs["player"]
        ...
        s = _state.setdefault(player, {"geese_bought_day0": False, "geese_bought_day5": False})

        # Buy geese
        if not s["geese_bought_day0"]:
            market_actions.append(["BUY_ANIMAL", "GOOSE", 2])
            s["geese_bought_day0"] = True
        if day >= 5 and not s["geese_bought_day5"]:
            market_actions.append(["BUY_ANIMAL", "GOOSE", 2])
            s["geese_bought_day5"] = True
        ...
        # Sell fertilizer at $90+
        fert_price = prices.get("FERTILIZER", 0)
        if shed.get("FERTILIZER", 0) > 0 and fert_price >= 90:
            market_actions.append(["SELL", "FERTILIZER", shed["FERTILIZER"]])

        # Sell melons
        melon_price = prices.get("MELON", 0)
        if shed.get("MELON", 0) > 0 and melon_price >= 150:
            sell_qty = min(shed["MELON"], 6)
            market_actions.append(["SELL", "MELON", sell_qty])
        elif shed.get("MELON", 0) > 0 and melon_price >= 100:
            sell_qty = min(shed["MELON"], 4)
            market_actions.append(["SELL", "MELON", sell_qty])

        # Farmer: COLLECT_FERTILIZER > FERTILIZE melons > water > harvest > plant
        ...
        harvest_ready = [(x, y, t) for x, y, t in all_tiles
                         ...
                         and (t.get("yield_units", 0) >= 6 or (day - t.get("planted_day", 0)) >= 10)]
