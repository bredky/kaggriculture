...
def wheat_grinder(obs):
    ...
    wheat_count = seeds.get("WHEAT", 0)
    needed = len(empty_tiles) + 5
    if wheat_count < needed:
        market_actions.append(["BUY_SEED", "WHEAT", needed - wheat_count])
    ...
    harvest_ready = [(x, y, t) for x, y, t in all_tiles
                     ...
                     and obs["day"] - t.get("planted_day", 0) >= 4]
    plantable = [(x, y) for x, y, t in all_tiles if t is None and seeds.get("WHEAT", 0) > 0]
