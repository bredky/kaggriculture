...
def wheat_egg_steady(obs):
    try:
        ...
        # Buy wheat seeds
        if seeds.get("WHEAT", 0) + wheat_count < 15:
            need = 15 - wheat_count - seeds.get("WHEAT", 0)
            if need > 0:
        ...
        if shed.get("FERTILIZER", 0) > 0 and fert_price >= 95:
            market_actions.append(["SELL", "FERTILIZER", shed["FERTILIZER"]])

        # Farmer: FEED > CARE > HARVEST > COLLECT_FERTILIZER > water wheat > harvest wheat > plant wheat
        cur_tile = farm["tiles"][fy][fx] if 0 <= fy < len(farm["tiles"]) and 0 <= fx < len(farm["tiles"][fy]) else None
