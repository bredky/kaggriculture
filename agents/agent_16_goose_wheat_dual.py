...
def goose_wheat_dual(obs):
    ...
    if day == 0 and s["geese_bought"] == 0:
        ...
        s["geese_bought"] = 2
    elif day == 3 and s["geese_bought"] < 4:
        market_actions.append(["BUY_ANIMAL", "GOOSE", 2])
        s["geese_bought"] = 4
    ...
    if fert_in_shed >= 3 and s["fert_sold_today"] < 4:
        sell_qty = min(fert_in_shed - (fert_in_shed % 3), 4 - s["fert_sold_today"])
        if sell_qty > 0:
