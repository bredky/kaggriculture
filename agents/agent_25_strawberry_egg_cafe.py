...
def strawberry_egg_cafe(obs):
    try:
        ...
        if seeds.get("STRAWBERRY", 0) + straw_count < 6:
            need = 6 - straw_count - seeds.get("STRAWBERRY", 0)
            if need > 0:
        ...
        # Sell strawberries: base 2 + shops bonus
        shops = obs.get("town", {}).get("unlocked_shops", [])
        straw_rate = min(4, 2 + len(shops) // 2)
        ...
        # Farmer: FEED > CARE > HARVEST geese > COLLECT_FERTILIZER > water straw > harvest straw > water wheat > harvest wheat
        cur_tile = farm["tiles"][fy][fx] if 0 <= fy < len(farm["tiles"]) and 0 <= fx < len(farm["tiles"][fy]) else None

        water_straw_urgent = [(x, y, t) for x, y, t in all_tiles
        ...
        water_wheat_urgent = [(x, y, t) for x, y, t in all_tiles
