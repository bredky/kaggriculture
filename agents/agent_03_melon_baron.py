...
def melon_baron(obs):
    ...
    if not late_game:
        needed = max(0, len(empty_tiles) + 5 - seeds.get("MELON", 0))
        if needed > 0:
            market_actions.append(["BUY_SEED", "MELON", needed])
    else:
    ...
    if melon_in_shed > 0 and melon_price >= 100 and s["melon_sold_today"] < 10:
        sell_qty = min(melon_in_shed, 10 - s["melon_sold_today"])
    ...
    if water_urgent:
    ...
    elif harvest_ready or wheat_harvest:
        candidates = harvest_ready if harvest_ready else wheat_harvest
        ...
        crop = None
        if not late_game and seeds.get("MELON", 0) > 0:
            crop = "MELON"
