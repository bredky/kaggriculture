...
def egg_factory(obs):
    ...
    s = _state.setdefault(player, {"geese_bought": 0})


    if day == 0 and s["geese_bought"] == 0:
        market_actions.append(["BUY_ANIMAL", "GOOSE", 3])
        s["geese_bought"] = 3
    elif day == 2 and s["geese_bought"] < 6:
        market_actions.append(["BUY_ANIMAL", "GOOSE", 3])
        s["geese_bought"] = 6
    ...
    if shed.get("FERTILIZER", 0) >= 5:
        market_actions.append(["SELL", "FERTILIZER", shed["FERTILIZER"] - (shed["FERTILIZER"] % 5)])
