...
def tomato_fertilizer_max(obs):
    ...
    s = _state.setdefault(player, {
        "tomato_sold_today": 0, "last_day": -1,
        "fertilized": set(), "fert_wave": 0
    })
    if day != s["last_day"]:
        s["tomato_sold_today"] = 0
        s["last_day"] = day
    ...
    if day == 0:
        if seeds.get("TOMATO", 0) < 10:
            market_actions.append(["BUY_SEED", "TOMATO", 10 - seeds.get("TOMATO", 0)])
    ...
    wheat_slots = max(0, len(empty_tiles) - tomato_needed)
    if wheat_slots > 0 and seeds.get("WHEAT", 0) < wheat_slots:
        market_actions.append(["BUY_SEED", "WHEAT", wheat_slots - seeds.get("WHEAT", 0)])

    if shed.get("TOMATO", 0) > 0 and s["tomato_sold_today"] < 10:
        sell_qty = min(shed["TOMATO"], 10 - s["tomato_sold_today"])
        market_actions.append(["SELL", "TOMATO", sell_qty])
        s["tomato_sold_today"] += sell_qty
    if shed.get("WHEAT", 0) > 0:
        market_actions.append(["SELL", "WHEAT", shed["WHEAT"]])

    tomato_tiles = [(x, y, t) for x, y, t in all_tiles
                    if isinstance(t, dict) and t.get("kind") == "PLANT" and t.get("crop") == "TOMATO"]
    fertilize_now = (day in (7, 10)) and shed.get("FERTILIZER", 0) > 0
    fertilize_targets = [(x, y, t) for x, y, t in tomato_tiles
                         if fertilize_now and (x, y, day) not in s["fertilized"]]
    ...
    if fertilize_targets:
        ...
        crop = None
        if tomato_needed > 0 and seeds.get("TOMATO", 0) > 0:
            crop = "TOMATO"
